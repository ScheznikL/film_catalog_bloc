...
class UserListBloc extends Bloc<UserListEvent, UserListState> {
  UserListBloc({
    ...
    on<LikeFilm>((event, emit) {
      ...
      _userRepository.saveUserData(UserData(liked: _userLikes, toWatch: _userWatch));
      emit(FilmLiked(userLikes: _userLikes));
    });

    on<UnLikeFilm>((event, emit) {
      _userLikes.remove(event.unlikedFilm);
      _userRepository.updateUserData(UserData(liked: _userLikes, toWatch: _userWatch));
    ...
    /*on<UserListsInit>((event, emit) {
      emit(UserListsInitState());
    });*/
